...
import { BarChart3, Star, AlertTriangle, CheckCircle2, RefreshCw, Clock, ArrowUpRight, Users, FileText, Search, Eye } from "lucide-react";
import { Line, Bar } from "react-chartjs-2";
import StatCard from "./StatCard";
// import { Eye, FileText, Users, Search, Star } from "lucide-react";
...
export default function OverviewTab({
    ...
    categories,
    analytics,
    totalViews, // all-time views from articles, not used for the 30d stat
    publishedArticles,
    pendingReview,
    draftCount,
    staleArticles,
    ...
    setActiveTab,
    users = [],
    timeSeries = [], // from the API
    }) {
    // Use provided timeSeries, fallback to analytics?.timeSeries, then empty
    const effectiveTimeSeries = timeSeries.length > 0 ? timeSeries : (analytics?.timeSeries ?? []);
    const ratingDist = analytics?.ratingDistribution ?? [];

    // Compute 30-day views and searches from the time series
    const views30d = useMemo(() => {
    return effectiveTimeSeries.reduce((sum, d) => sum + (d.views || 0), 0);
    }, [effectiveTimeSeries]);

    const searches30d = useMemo(() => {
    return effectiveTimeSeries.reduce((sum, d) => sum + (d.searches || 0), 0);
    }, [effectiveTimeSeries]);

    const totalSearchesAllTime = analytics?.totalSearches ?? 0;

    // Category view stats (all-time)
    ...
    labels: effectiveTimeSeries.map((d) => d.date || ""),
    datasets: [
        {
        label: "Views",
        data: effectiveTimeSeries.map((d) => d.views || 0),
        ...
        {
        label: "Searches",
        data: effectiveTimeSeries.map((d) => d.searches || 0),
    ...
    <div className="space-y-6">
        ...
        <StatCard
            ...
            value={views30d.toLocaleString()} // <-- 30-day views from timeSeries
            sub="Last 30 days"
            badgePct={analytics?.viewsChangePct}
            color="#F22F46"
            sparkline={effectiveTimeSeries.slice(-14)}
            sparklineKey="views"
            icon={Eye}
        />
        <StatCard
        ...
        <StatCard
            label="Articles"
            value={publishedArticles.length.toLocaleString()}
            sub="Published"
            color="#00A368"
            icon={FileText}
        />
        ...
        <StatCard
            ...
            value={totalSearchesAllTime.toLocaleString()}
            sub="All-time searches"
            color="#E87722"
            icon={Search}
        />
        <StatCard
            label="Searches (30d)"
            value={searches30d.toLocaleString()}
            sub="Last 30 days"
