...
export default function ForgotPasswordPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12" style={{ background: "#F4F4F6" }}>
          ...
          {!sent ? (
            <>
              <div className="text-center mb-7">
                ...
                <p className="text-sm" style={{ color: "#696E7A" }}>Enter your work email and we'll send you a link to reset your password.</p>
              </div>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@healthtech.co.ke" autoFocus className="w-full px-3.5 py-2.5 text-sm rounded-md border outline-none transition-all" style={{ borderColor: "#E1E3EA", color: "#121C2D" }} />
                </div>
                <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-md text-sm font-medium disabled:opacity-70 transition-opacity" style={{ background: "#F22F46", color: "white" }}>
                  {loading ? (<><Loader2 size={15} className="animate-spin" /> Sending link…</>) : "Send reset link"}
                </button>
              </form>
            </>
          ) : (
            <div className="text-center py-4">
              ...
              <p className="text-sm mb-2" style={{ color: "#696E7A" }}>
                If an account exists for <strong style={{ color: "#121C2D" }}>{email}</strong>, we've sent a link to reset your password.
              </p>
            </div>
          )}
        </div>
