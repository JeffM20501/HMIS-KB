...
from articles.test.helper import create_user, create_category, create_article, unique_slug
from articles.models.article import Article, Category
...
class SearchLogTest(BaseAPITestCase):
    """Test search log functionality with public access."""
    ...
    def setUp(self):
        ...
        self.admin = create_user(role='admin', username='admin', password='12345')
        self.category = create_category()   # now unique name
        self.article = create_article(self.user, self.category, status='published')

    def test_anonymous_search_creates_log_without_user(self):
    ...
    def test_anonymous_search_deduplication(self):
        url = reverse('articles:article-list')
    ...
    def test_anonymous_search_different_queries_logged_separately(self):
        url = reverse('articles:article-list')
        self.client.get(url, {'search': 'first'})
        self.client.get(url, {'search': 'second'})
        logs = SearchLog.objects.all()
        self.assertEqual(logs.count(), 2)

    # ---------- AUTHENTICATED SEARCH LOGGING ----------
    def test_authenticated_search_creates_log_with_user(self):
        self._login(self.user)
        url = reverse('articles:article-list')
        response = self.client.get(url, {'search': 'auth test'})
        self.assertEqual(response.status_code, 200)
        log = SearchLog.objects.filter(query='auth test').first()
        self.assertIsNotNone(log)
        self.assertEqual(log.user, self.user)

    def test_authenticated_search_deduplication(self):
        self._login(self.user)
        url = reverse('articles:article-list')
        self.client.get(url, {'search': 'dup'})
        self.client.get(url, {'search': 'dup'})
        logs = SearchLog.objects.filter(query='dup')
        self.assertEqual(logs.count(), 1)
    ...
    def test_user_cannot_view_others_logs(self):
        other_user = create_user(role='viewer', username='other')
    ...
    def test_search_log_stats_endpoint_admin_only(self):
        self._login(self.admin)
